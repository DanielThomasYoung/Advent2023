...
def recursive_check(
    line, targets, current_size=0, current_index=0, current_target=0, recursive_sum=0
):
    ...
    for index in range(current_index, len(cloned_line)):
        ...
        if cloned_line[index] == "#":
            ...
            cloned_line[index] = "."
            recursive_sum += recursive_check(
                cloned_line, targets, current_size, index, current_target
            )
            cloned_line[index] = "#"
            current_size += 1
            if current_size > int(targets[current_target]):
